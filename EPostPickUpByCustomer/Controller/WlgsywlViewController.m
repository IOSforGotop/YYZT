...
#import "Utility.h"
#import "WlgsywlViewController.h"
...
@interface WlgsywlViewController () <UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, MBProgressHUDDelegate, InputToolBarDelegate> {
    MBProgressHUD *HUD;
}
...
//物流公司  来件数  取件数  退回数  自行处理数  结存数
@property (nonatomic, strong) NSMutableArray *wlgs;
...
@implementation WlgsywlViewController
...
- (void) ini{
    ...
    self.navigationItem.backBarButtonItem = backItem;
      ...
      self.cellNum = 0;
    self.wlgs  = [[NSMutableArray alloc] init];
...
- (IBAction)search:(id)sender {
    ...
    if ([Utility netState]) {
        ...
        NSString *getxmldata2=[Webconn Xmlappend:sDic.JSONString params:@"wlgsywltj" params1:@"http://service.search.gnzq.com" params2:@"PostService"];    //调用自定义方法  传入 方法名 命名空间   url
  ...
  //      NSLog(@"lala1返回的soap信息是：[%@]\n",backresult2);
            ...
            
        {
            ...
            {
                nn9 = [NSString stringWithFormat:@"%@",[[member objectForKey:@"WLGS"] description]];
                ...
                [self.wlgs addObject:nn9];
                [self.ljs addObject:nn10];
                [self.qjs addObject:nn11];
                [self.ths addObject:nn12];
                [self.zxcls addObject:nn13];
                [self.jcs addObject:nn14];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {   //自定义显示格式
    ...
    cell.postLabel.text = [NSString stringWithFormat:@"物流公司：%@",  self.wlgs[indexPath.row]];
