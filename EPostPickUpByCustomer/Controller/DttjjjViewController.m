...
@interface DttjjjViewController () <UITextFieldDelegate,UITableViewDataSource, UITableViewDelegate, CustomIOSAlertViewDelegate, MBProgressHUDDelegate> {
    MBProgressHUD *HUD;
}
...
@implementation DttjjjViewController
...
- (void) ini{                           //只要一进入这个页面 立刻调用查询接口 获取未被交接的数量
    ...
    if ([Utility netState]) {
        ...
        dic = [[NSDictionary alloc]initWithObjectsAndKeys:jgbh, @"V_JGBH",czygh,@"V_CZYGH", @"4",@"C_YJZT",nil];  //站点退回的状态
        
   //     NSLog(@"444[111%@]\n",dic);
   //     NSLog(@"555[111%@]\n",dic.JSONString);
        NSString *getxmldata51=[Webconn Xmlappend:dic.JSONString params:@"getDTPostInfo" params1:@"http://service.search.gnzq.com" params2:@"PostService"];    //调用自定义方法  传入 方法名 命名空间   url
