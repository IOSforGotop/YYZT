...
@interface TJDJViewController () <UITableViewDataSource, UITableViewDelegate, UITextFieldDelegate, CustomIOSAlertViewDelegate, /*PassQRCodeDelegate,*/ ZBarReaderDelegate, MBProgressHUDDelegate,ZBarReaderViewDelegate> {
    MBProgressHUD *HUD;
...
@implementation TJDJViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    self.tabBarController.tabBar.hidden = YES;
    [self ini];
    [self getsegment];  //定义自定义滑块
    
    [self tuihuiyychaxun];//调用查看退回原因
    [self zxclyychaxun];//自行处理原因接口
...
-(void)zxclyychaxun
{
    ...
    if ([Utility netState]) {
        NSDictionary *zxclyychaxundic;
        
        zxclyychaxundic = [[NSDictionary alloc]initWithObjectsAndKeys:jgbh, @"V_JGBH", czygh, @"V_CZYGH", nil];  //站点退回的状态
        
        NSString *zxclyygetxmldata51=[Webconn Xmlappend:zxclyychaxundic.JSONString params:@"queryZxclyy" params1:@"http://service.search.gnzq.com" params2:@"PostService"];    //调用自定义方法  传入 方法名 命名空间   url
        
        //@"http://192.168.201.110:7602/gnzqService/service/PostService"
        NSError *zxclyyerror51 = nil;
        NSData *zxclyydata51 = [NSURLConnection sendSynchronousRequest:zxclyygetxmldata51 returningResponse:nil error:&zxclyyerror51];         if (zxclyydata51 == nil) {
            NSLog(@"send request failed: %@", zxclyyerror51);
            // return nil;
        }
        
        NSString *zxclyyresponse51 = [[NSString alloc] initWithData:zxclyydata51 encoding:NSUTF8StringEncoding];
        
        //NSLog(@"response: %@", zxclyyresponse51);
        
        
        NSString *zxclyygetdata51=[Webconn decodexml:zxclyyresponse51];
        
        NSString *zxclyybackremark51=[Webconn jsondecoderemark:zxclyygetdata51];
     //   NSLog(@"lala返回的soap信息是：[%@]\n",zxclyybackremark51);
        
        NSString *zxclyybackresult51=[Webconn jsondecoderesult:zxclyygetdata51];
    //    NSLog(@"lala1返回的soap信息是：[%@]\n",zxclyybackresult51);
        
        
        
        
        if([zxclyybackresult51 isEqualToString:@"F0"])
            
        {
            ...
            //NSLog(@"/////////////////////////解析退回原因接口 /////////////////////////\n\n");
            SBJsonParser *zxclyyparser18 = [[SBJsonParser alloc] init];
            NSError *zxclyyerror18 = nil;
            
            
            NSMutableDictionary *zxclyyroot = [[NSMutableDictionary alloc]initWithDictionary:[zxclyyparser18 objectWithString:zxclyybackremark51 error:&zxclyyerror18]];//获取根节点 只需要获取一次
            
            
            //注意转换代码
            SBJsonWriter *zxclyyjsonWriter = [[SBJsonWriter alloc] init];
            
            NSString *zxclyyjsonStringhello = [zxclyyjsonWriter stringWithObject:zxclyyroot];
            
            NSMutableDictionary *zxclyydicUserInfo = [zxclyyroot objectForKey:@"rows"];
          
            
            
            _zxclyycarray=[[NSMutableArray alloc]initWithCapacity:100];
            
            
            int i=0;
            for(NSMutableDictionary * member  in zxclyydicUserInfo)
            {
                
            //    NSLog(@"%@[%d]",[[member objectForKey:@"SM"] description],i++);
                
                
                [_zxclyycarray addObject:[[member objectForKey:@"SM"] description]];
...
- (void)getsegment  //自行添加自定义分段  控件
{
    
    
    self.segmentedControl.tintColor = [UIColor colorWithRed:49.0 / 256.0 green:148.0 / 256.0 blue:108.0 / 256.0 alpha:1];
   
    self.segmentedControl.selectedSegmentIndex = 0;//默认选中的按钮索引
      NSDictionary *attributes = [NSDictionary dictionaryWithObjectsAndKeys:[UIFont boldSystemFontOfSize:18],NSFontAttributeName,[UIColor blackColor], NSForegroundColorAttributeName,nil];  //这个是设置标签的字体大小
    [self.segmentedControl setTitleTextAttributes:attributes forState:UIControlStateNormal];    //这个是设置未被按下的时候的  属性
...
- (IBAction)searchByPostNum:(id)sender {
    ...
    if ([self.postNumTextField.text isEqual:@""] || !self.postNumTextField.text) {
        ...
        if ([Utility netState]) {
            ...
            NSDictionary *dic = [[NSDictionary alloc]initWithObjectsAndKeys:jgbh, @"V_JGBH", self.postNumTextField.text, @"V_YJHM", @"", @"V_SJRDH", @"1", @"C_JSBZ", @"0", @"C_QSBZ",@"",@"C_JQBZ",@"1",@"C_YJZT",nil];   //状态为1 表示待取件
   ...
   //         NSLog(@"lala返回的soap信息是：[%@]\n",backremark4);
    ...
    //        NSLog(@"lala1返回的soap信息是：[%@]\n",backresult4);
                ...
                
            {
                ...
                //新增自行处理原因 根据两个标签的选择不同 进行发送调用
                 else if (self.exist == 1) {
                     [self.tempUserName addObject:sjrxm];
                     [self.tempPostNum addObject:yjhm];
                     [self.tempPhoneNum addObject:sjrdh];
                     self.cellNum++;    //行列数  +1 表示 选择项到了下一行
                     
                                       //历史性重大突破
                     _zxclyyalerView = [[UIAlertView alloc]initWithTitle:@"自行处理原因" message:nil delegate:self cancelButtonTitle:nil otherButtonTitles: nil];
                     for(NSString *title in _zxclyycarray) {
                         
                         [_zxclyyalerView addButtonWithTitle:title];
                     }
                     
                     [_zxclyyalerView addButtonWithTitle:@"取消"];
                     _zxclyyalerView.cancelButtonIndex=[_zxclyycarray count];
                     //设置将某一个按钮设置为取消按钮@property(nonatomic) NSInteger cancelButtonIndex;
                     [_zxclyyalerView show];   //这一段代码是关键性的一步  实现了如何进行在alert上数组的添加
                     //历史性重大突破
                     ...
                     [self.tableView reloadData];  //然后页面重新加载
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    ...
    else if((self.exist==1)&&(alertView==_zxclyyalerView)){
        
      //  NSLog(@"按键[%d]被按下\n",buttonIndex);
        
        if (buttonIndex != [_zxclyycarray count]) {
            
            if (buttonIndex == 3) {
                CustomIOSAlertView* cusAlert = [[CustomIOSAlertView alloc] init];
                
                [cusAlert setContainerView:[self zxclcreateDemoView]];
                [cusAlert setButtonTitles:[NSMutableArray arrayWithObjects:@"取消", @"确定", nil]];
                ...
                //[self returnPost:(int)buttonIndex andOtherReason:@""];
                [self dealbyselfputPost:(int)buttonIndex dealbyselfputandOtherReason:@""];
            }
        }
        
        else {
            
            //NSLog(@"111222");                               //这个按键是 0
            if(self.cellNum != 0) {                           //如果点击了取消按钮
                [self.tempPhoneNum removeLastObject];
                [self.tempPostNum removeLastObject];
                [self.tempUserName removeLastObject];
...
- (UIView *)zxclcreateDemoView

{
    UIView *zxcldemoView =  [[UIView alloc] initWithFrame:CGRectMake(0, 0, 290, 120)];
    
    UILabel *zxcltitleLabel =    [[UILabel alloc] initWithFrame:CGRectMake(10, 10, 270, 21)];
    zxcltitleLabel.textColor = [UIColor orangeColor];
    zxcltitleLabel.text = @"处理原因";
    UILabel *zxcllable1 = [[UILabel alloc] initWithFrame:CGRectMake(10, 36, 270, 21)];
    zxcllable1.text = @"其他";
    UILabel *zxcllable2 = [[UILabel alloc] initWithFrame:CGRectMake(10, 61, 270, 21)];
    zxcllable2.text = @"其他处理原因";
    
    UITextField *zxcluserNameTextField  = [[UITextField alloc] initWithFrame:CGRectMake(10, 87, 270, 23)];
    zxcluserNameTextField.placeholder = @"请输处理原因";
    zxcluserNameTextField.delegate=self;
    [zxcluserNameTextField  setBorderStyle:UITextBorderStyleRoundedRect];
    
    UIColor *color = [[UIColor alloc] initWithRed:200.0/255.0 green:200.0/255.0 blue:200.0/255.0 alpha:1.0];
    zxcldemoView.backgroundColor = color;
    zxcltitleLabel.backgroundColor = color;
    zxcllable1.backgroundColor = color;
    zxcluserNameTextField.backgroundColor = color;
    zxcllable2.backgroundColor = color;
    
    
   
    [zxcldemoView addSubview:zxcltitleLabel];
    [zxcldemoView addSubview:zxcllable1];
    [zxcldemoView addSubview:zxcluserNameTextField];
    [zxcldemoView addSubview:zxcllable2];
...
{   //这里是根据不同的接口  进行调用不同的 处理方式 一个是退件登记  一个是自行处理
    ...
    if(self.exist == 0){
        ...
        if (buttonIndex == 1) {        //这里是 点击了 其他 原因以后 进行确定的  按钮点击  显示的  如果点击的是 取消 则会删除本身这一行
        [self dealbyselfputPost:3 dealbyselfputandOtherReason:((UITextField *)alertView.containerView.subviews[2]).text];
...
//    NSLog(@"[%d]\n",self.exist);
    ...
    if ([Utility netState]) {
        ...
        NSString *getxmldata41=[Webconn Xmlappend:dic.JSONString params:@"backPost" params1:@"http://service.search.gnzq.com" params2:@"PostService"];    //调用自定义方法  传入 方法名 命名空间   url
...
//调用自行处理接口
- (void)dealbyselfputPost:(int)dealbyselfreturnReason dealbyselfputandOtherReason:(NSString *)dealbyselfotherReason {      //这个是点击退件原因 的时候进行一个数据库插入操作
   // NSLog(@"[调用自行处理接口]\n");
  //  NSLog(@"[%d]\n",self.exist);
    ...
    if ([Utility netState]) {
        ...
        if(dealbyselfreturnReason != 3){
            NSString *string22 = [[NSString alloc] initWithFormat:@"%d",(dealbyselfreturnReason+1)];
            dic22 = [[NSDictionary alloc]initWithObjectsAndKeys:jgbh, @"V_JGBH", _tjyjlsh, @"V_YJLSH", string22, @"C_ZXCLYY", @"", @"V_QTZXCLYY",nil];
        }
        else{
            
            NSString *string122 = [[NSString alloc] initWithFormat:@"%d",(dealbyselfreturnReason+1)];
            dic22 = [[NSDictionary alloc]initWithObjectsAndKeys:jgbh, @"V_JGBH",_tjyjlsh, @"V_YJLSH", string122, @"C_ZXCLYY", dealbyselfotherReason, @"V_QTZXCLYY",nil];
        }
        
        NSString *getxmldata4122=[Webconn Xmlappend:dic22.JSONString params:@"destroyPost" params1:@"http://service.search.gnzq.com" params2:@"PostService"];    //调用自定义方法  传入 方法名 命名空间   url
        
        
        //@"http://192.168.201.110:7602/gnzqService/service/PostService"
        NSError *error4122 = nil;
        NSData *data4122 = [NSURLConnection sendSynchronousRequest:getxmldata4122 returningResponse:nil error:&error4122];
        if (data4122 == nil) {
            NSLog(@"send request failed: %@", error4122);
            // return nil;
        }
        
        NSString *response4122 = [[NSString alloc] initWithData:data4122 encoding:NSUTF8StringEncoding];
       // NSLog(@"response: %@", response4122);
        
        
        
        NSString *getdata4122=[Webconn decodexml:response4122];
        
        NSString *backremark4122=[Webconn jsondecoderemark:getdata4122];
    //    NSLog(@"lala返回的soap信息是：[%@]\n",backremark4122);
        
        NSString *backresult4122=[Webconn jsondecoderesult:getdata4122];
    //    NSLog(@"lala1返回的soap信息是：[%@]\n",backresult4122);
            ...
            
        {
            ...
            UIAlertView *alter = [[UIAlertView alloc] initWithTitle:@"提示" message:@"自行处理成功" delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil];
            [alter show];
